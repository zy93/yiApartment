...
- (void)check
{
    ...
    if ([self.fistInput isEqualToString:self.lastInput]) {
            ...
            [[GXNetWorkManager shareInstance] getInfoWithInfo:dictory path:@"/user/forgetpaypwd" success:^(NSMutableDictionary * dict) {
                if ([dict[@"code"] isEqualToString:@"0"]) {
                    
                    NSArray * ctrlArray = self.navigationController.viewControllers;
                    
                    [weakself.navigationController popToViewController:[ctrlArray objectAtIndex:ctrlArray.count - 2] animated:YES];

                    [self showAlertWithString:dict[@"message"]];
                    [[NSUserDefaults standardUserDefaults] setObject:@"1" forKey:@"UPassWord"];
                    
                }else{
                    [self showAlertWithString:@"上传密码失败"];
                }
            } failed:^{
                NSLog(@"未知错误");
            }];

            NSLog(@"密码为：%@", self.lastInput);
