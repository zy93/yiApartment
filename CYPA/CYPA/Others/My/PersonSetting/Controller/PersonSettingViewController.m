...
-(void)makeData {
    ...
    [[GXNetWorkManager shareInstance] getInfoWithInfo:dictory path:@"/user/getUserInfoByUID" success:^(NSMutableDictionary * dict) {
        ...
        if ([dict[@"code"] isEqualToString:@"0"]) {
            ...
            [person setValuesForKeysWithDictionary:dict[@"data"]];
                        
            [self.headBT sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",BaseImageURL, person.UHeadPortrait]] forState:normal placeholderImage:[UIImage imageNamed:@"970"]];
            
            self.nameTF.text = person.UNickName;
            
            if ([person.USex isEqualToString:@"男"]) {
            ...
            self.ageTF.text = person.UAge;
            
            if (person.UHome.length == 0) {
                
            } else {
                NSArray * array = [person.UHome componentsSeparatedByString:@"-"];
                self.countryTF.text = array[0];
                self.cityTF.text = array[1];
            }
            self.hobbyLB.text = person.UHobby;
            self.constellationLB.text = person.UConstellation;
            if (person.USignaTure.length != 0) {
                self.placeholderLabel.hidden = YES;
            }
            self.introTextView.text = person.USignaTure;
...
-(void)setupViews{
    ...
    headLabel1.text = @"求真相";
    headLabel1.font = [UIFont systemFontOfSize:13];
    ...
    self.ageTF.font = [UIFont systemFontOfSize:15];
    ...
    self.cityTF.font = [UIFont systemFontOfSize:15];
    ...
    self.hobbyLB.font = [UIFont systemFontOfSize:15];
    [self.view addSubview:self.hobbyLB];
    
    UITapGestureRecognizer * tap1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectHobby)];
    self.hobbyLB.userInteractionEnabled = YES;
    [self.hobbyLB addGestureRecognizer:tap1];
    ...
    [self.hobbyLB mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(SIZE_SCALE_IPHONE6(72), SIZE_SCALE_IPHONE6(20)));
    }];
    ...
    self.constellationLB.font = [UIFont systemFontOfSize:15];
    ...
    [self.constellationLB mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.ageTF.mas_left);
        make.right.mas_equalTo(self.hobbyLB.mas_right);
        make.height.mas_equalTo(self.hobbyLB.mas_height);
    }];
    
    UITapGestureRecognizer * tap2 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectConstellation)];
    self.constellationLB.userInteractionEnabled = YES;
    [self.constellationLB addGestureRecognizer:tap2];
...
-(void)publishAction{
    ...
    if ([self.nameTF.text isEqualToString:@""] || self.sexChange == 0 || [self.ageTF.text isEqualToString:@""] || [self.countryTF.text isEqualToString:@""] || [self.cityTF.text isEqualToString:@""] || [self.hobbyLB.text isEqualToString:@""] || [self.constellationLB.text isEqualToString:@""] || [self.introTextView.text isEqualToString:@""]) {
        ...
        [self showAlertWithString:@"请补充您的资料"];
...
//                NSLog(@"%@", dictory);
                ...
                [[GXNetWorkManager shareInstance] getInfoWithInfo:dictory path:@"/user/edit" success:^(NSMutableDictionary * dict1) {
                    ...
                    if ([dict1[@"code"] isEqualToString:@"0"]) {
                        ...
                        [dictory1 setValue:[[NSUserDefaults standardUserDefaults] objectForKey:@"phone"] forKey:@"phone"];
//                        NSLog(@"%@", self.phone);
...
//                            NSLog(@"%@",dict2);
                            if ([dict2[@"code"] isEqualToString:@"0"]) {
                                ...
                                [[NSUserDefaults standardUserDefaults] setValue:self.citizenModel.RoomNo forKey:@"RoomNo"];
//                            
                                [[NSUserDefaults standardUserDefaults] setValue:self.citizenModel.RoomID forKey:@"RoomID"];
                                [[NSUserDefaults standardUserDefaults] setObject:self.citizenModel.CustID forKey:@"CustID"];
                                [[NSUserDefaults standardUserDefaults] setObject:self.citizenModel.ApartmentID forKey:@"ApartmentID"];
                                [[NSUserDefaults standardUserDefaults] setObject:self.citizenModel.UHeadPortrait forKey:@"UHeadPortrait"];
                                [[NSUserDefaults standardUserDefaults] setObject:self.citizenModel.UNickName forKey:@"UNickName"];
                                [[NSUserDefaults standardUserDefaults] setObject:self.citizenModel.UPassWord forKey:@"UPassWord"];
                            }else{
                                NSLog(@"##获取用户数据出错");
                            }
                        } failed:^{
                            [self showAlertWithString:@"获取用户信息失败"];
                        }];
                    }else {
                        NSLog(@"个人设置失败");
                    }
